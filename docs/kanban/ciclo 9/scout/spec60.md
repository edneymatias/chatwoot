...
- [ ] **[Fase 6 — Busca Vetorial na Base de Conhecimento (RAG)](06-rag-knowledge-search/spec67.md)**: Refatora a injeção cega de conteúdo (`AgentRunner#build_knowledge_instructions`) por recuperação sob demanda via embeddings — reaproveita o pipeline de extração já existente (`ScoutKnowledgeSource`/`ProcessJob`, Fase 1), inspirado na solução do Captain (`pgvector`/`neighbor`, já presentes no `Gemfile` base).
- [ ] **[Fase 7 — Arquitetura de Guardrails do System Prompt](07-system-prompt-guardrails/spec-preview.md)** *(preview)*: Camada fixa de guardrails do system prompt do Scout (escopo, anti-alucinação, anti-falsa-promessa, fallback de handoff), inspirada na arquitetura do Captain (`Captain::Llm::SystemPromptsService#assistant_response_generator`), aditiva às instruções configuráveis pelo operador em `Scout#system_prompt`.
- [ ] **[Fase 8 — Inteligência de Funil: Estágios & Campos de Qualificação](08-required-qualification-attributes/spec-preview.md)** *(preview)*: Injeção dos estágios configurados do Funil (qualificado/desqualificado) e dos campos obrigatórios por estágio (`PipelineStageRequiredField`) no system prompt, com enforcement em `MoveOpportunityStage`.
- [ ] **[Fase 9 — UI na Conversa](09-in-conversation-ui/spec68.md)**: Componentes de UI na Conversa (Badge de Status do Scout, botão Pausar/Retomar e link para o card no Kanban).
- [ ] **[Fase 10 — Follow-up, Telemetria & E2E](10-follow-up-telemetry-e2e/spec69.md)**: Job de Follow-up / Re-engajamento (`Scout::FollowUpJob`, a criar do zero — seção 5), telemetria de tokens/cota e testes ponta a ponta.
- [ ] **[Fase 11 — Auditor de Resposta (pós-v1)](11-response-auditor/spec-preview.md)** *(condicional a métricas do v1)*: Segunda chamada de LLM que audita a resposta final antes da persistência (classificador de ação e/ou detector de falsa-promessa), inspirada em `Captain::Conversation::V1ActionClassifier`/`V1FalsePromiseHandler`. Só entra em implementação se a telemetria da Fase 10 mostrar handoffs perdidos ou promessas futuras não cumpridas no v1; depende do ponto único de interceptação da resposta final estabelecido na Fase 7 (ver nota nesse spec).
