...
- [ ] **[Fase 34 — Nota Interna de Transferência Não Renderizada na Timeline do Dashboard (preview)](34-dashboard-transfer-note-not-rendering/spec-preview.md)** *(achado único, registrado como achado adicional fora de escopo da Fase 32 — conversation_id 71392/display_id 45006, conta 2 "Dens Odontologia"; **sem causa raiz identificada** e não reproduzido novamente; preview de baixa confiança, não avançar para `/speckit-specify` sem reprodução/evidência nova)*: nota privada automática de transferência (`HandoffService#create_transfer_note`) confirmada íntegra no banco (`Message#id 1407500`, `private: true`, `status: "read"`) mas não renderizada na timeline do dashboard mesmo após hard reload; inversão sistêmica de timestamp entre nota e mensagem pública (4/4 notas do sistema) descartada como explicação exclusiva; suspeita não confirmada de causa no frontend (componente de lista de mensagens lidando com `private: true` + `sender_type: null`) — sem reprodução ao vivo disponível.
- [ ] **[Fase 35 — Laço de Repair do Auditor Vaza Narrativa Interna para o Cliente e Duplica a Chamada de Handoff (preview)](35-response-auditor-repair-loop-narrative-leak/spec-preview.md)** *(problema real identificado em simulação, conversation_id 134/display_id 133, conta 1 "Acme Inc", Scout "VItória"; especificação completa e implementação ficam para `/speckit-specify`)*: quando o modelo decide um handoff via `handover_to_human` (caminho 1) e a conversa ainda está `pending` no momento da auditoria, `ResponseAuditor#audit` pula o classificador de ação (Fase 12) mas **não** pula `check_claim_consistency` — se o `ClaimConsistencyService` marcar a resposta (já correta, com a tool executada com sucesso) como inconsistente, dispara `execute_repair`, que injeta no mesmo `chat` uma instrução de "sua resposta anterior não foi executada" e faz o modelo chamar `handover_to_human` de novo, sobrescrevendo a mensagem pública e o motivo da nota interna com uma narrativa de "correção de turno anterior"/desculpa por confusão que o cliente nunca viu. Caminho de código totalmente diferente do classificador da Fase 33 (nem chega a rodar aqui). Inclui achado relacionado, fora do escopo desta fase: ambiguidade da persona configurável sobre pergunta de preço contar como "dúvida administrativa" — resolúvel pelo operador via `Scout#persona`, sem mudança de código.
- [ ] **[Fase 36 — "Testar" no Editor de Ferramenta Externa Envia o Valor Mascarado do Segredo, Nunca a Credencial Real (preview)](36-scout-tool-test-uses-masked-secret/spec-preview.md)** *(problema real identificado ao investigar a Fase 35, conta 1 "Acme Inc", Scout "Vitória"; especificação completa e implementação ficam para `/speckit-specify`)*: `masked_auth_headers` (`scout_tool.rb:54-68`) nunca devolve o segredo real ao frontend — correto por design — mas só `update`/`create` sabem reconciliar o valor mascarado de volta com o segredo salvo (`apply_credentials_update`, `scout_tool.rb:70-78`); a action `test` (`scout_tools_controller.rb:47-66`, rota `collection` sem `id`) nunca carrega o `ScoutTool` existente e repassa o literal `'••••••••'` do formulário direto para o `HttpRequestExecutor`, então testar uma ferramenta já configurada sem redigitar a credencial sempre falha com 401/403 — independente da credencial salva estar correta.
