...
- [ ] **[Fase 18 — Correspondência Desfecho-Estágio no Funil (preview)](18-funnel-outcome-stage-matching/spec-preview.md)** *(problema real diagnosticado em três conversas de produção — sem precedente no Captain, que não tem noção de Oportunidade/Kanban)*: `SystemPromptsService#funnel_section` já inclui a descrição de cada estágio configurado no contexto do modelo, mas nenhuma diretriz instrui o Scout a comparar ativamente o desfecho de um turno (recusa, adiamento, confirmação de agendamento) contra essas descrições para decidir uma transição — hoje as descrições são só contexto passivo. Direção desqualificação: lead recusou agendar ("não, volto depois"), o `reasoning` do modelo concluiu "não há ação comercial adicional a ser tomada", e a Oportunidade nunca saiu do estágio padrão apesar da descrição do estágio "Falhou" cobrir exatamente esse cenário. Direção qualificação (2 casos): lead confirma horário, Scout responde "está agendada"/"está reservado" ao cliente, mas nunca chama `move_opportunity_stage` para o estágio qualificado nem dispara o handoff automático correspondente — ninguém da equipe fica sabendo do agendamento. Como esse segundo sintoma é o padrão `false_completed_action` que a Fase 12 audita, `feature_response_auditor` foi ativado no Scout de teste como mitigação reativa de curto prazo enquanto a causa raiz (Fase 18) não é tratada. Tratamento definitivo adiado para o momento oportuno, a critério do operador.
- [ ] **[Fase 19 — Identidade do Contato & Label de Conversa (preview)](19-contact-identity-and-conversation-labeling/spec-preview.md)** *(dois temas com lógica nova, por isso separados da Fase 18, que é só prompt)*: **Tema 1** — contato do widget do site sem nome informado recebe um nome gerado automaticamente (`Haikunator.haikunate(1000)`, `ContactInboxWithContactBuilder#contact_name`, formato `adjetivo-substantivo-número`), mas o Scout nunca questiona isso nem pergunta o nome real por educação; proposta: helper determinístico detectando o padrão (só possível no site; em WhatsApp/Instagram o nome vem do canal e só dá pra tratar por julgamento do modelo) + contexto condicional no `SystemPromptsService` + reuso do `update_contact` já existente para persistir. **Tema 2** — gap de paridade com o catálogo nativo do Captain (`config/agents/tools.yml`): falta uma tool `add_label_to_conversation` no Scout; revisão do mesmo catálogo descartou `update_priority` (sem motivação de negócio) e uma tool de nota no contato (já coberto — `ContactNotesService` já grava em `contact.notes` no handoff).
- [ ] **[Fase 20 — Reavaliação do Handoff Automático de Qualificação (preview)](20-automatic-handoff-reevaluation/spec-preview.md)** *(bug real de código encontrado em sessão de debugging — `trigger_qualification_handoff` despachava a pergunta do modelo e transferia no mesmo turno, sem esperar resposta)*: `AgentRunner#qualification_handoff_needed?`/`#trigger_qualification_handoff` (Fase 09) é o único handoff do Scout que dispara sem nenhum julgamento de LLM — mecânico, no instante em que a Oportunidade entra no estágio qualificado. Pesquisa dedicada confirmou que o Captain não tem mecanismo análogo (arquitetura sem conceito de Oportunidade/Kanban), mas segue um padrão diretamente aplicável — nunca enviar o texto redigido pelo modelo junto de um handoff, só a mensagem fixa (`enterprise/app/jobs/captain/conversation/message_builder.rb#create_handoff_message`) — já normalizado no Scout como fix imediato de código em 2026-08-30, alinhando o caminho mecânico ao mesmo padrão que o caminho de handoff direto (`handover_to_human`) já seguia. A questão de fundo fica para esta fase: manter o mecanismo mecânico (rede de segurança contra a evidência original da Fase 18, conversation_id 20/22 — Scout confirmava agendamento sem nunca notificar um humano), substituí-lo por uma rede de segurança atrasada, ou eliminá-lo em favor de confiar a decisão inteiramente ao LLM, como o Captain faz por arquitetura. Tratamento definitivo adiado para o momento oportuno, a critério do operador.
