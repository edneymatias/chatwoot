...
- *(scout)* Introduce Scout core data model, migrations, and LLM client
- *(scout)* Implement native tools and message pipeline
- *(scout)* Harden production encryption for swarm deployment
- *(scout)* Implement external REST and webhook LLM tool
- *(scout)* Implement commercial UI with knowledge sources and RAG playground
- *(scout)* Add account-level LLM configuration and gating
- *(scout)* Implement RAG knowledge search and system prompt guardrails architecture
- *(scout)* Implement tool testing, response template shaping and playground tool execution
- *(scout)* Fix tool headers persistence, test payload handling and response truncation
- *(scout)* Implement http authentication and visual parameter builder for custom tools
- *(scout)* Add funnel qualification gate and required attributes configuration
- *(scout)* Implement opportunity continuity detection for conversations and automation rules
- *(scout)* Implement human handoff on manual intervention
- *(scout)* Enforce structured response schema and enhance reliability
- *(scout)* Add response auditor for handoff and claim consistency
- *(scout)* Implement funnel outcome-stage matching and conversational pacing
- *(scout)* Implement contextual natural handoff message
- *(scout)* Add contact identity detection and phone collection with merge-aware handoff safety
- *(scout)* Implement automated follow-up nudges, rescue handoff, and kanban badge
- *(scout)* Implement non-sales intent recognition and immediate handoff
- *(kanban)* Add unread message indicator for post-handoff opportunities
- *(scout)* Add audience targeting to gate scout engagement
- *(scout)* Implement opportunity value estimation by interest attribute
- *(scout)* Implement scout overview recent conversations and visual alignment
- *(erp)* Implement ERP integration foundation and contact data card
- *(erp)* Refine contact card UI and harden phone matching tolerance

### 🐛 Bug Fixes

- *(scout)* Close handoff and claim-consistency reliability gaps
- *(specs)* Renumber scout spec-kit docs to resolve collision with main
- *(whatsapp)* Render liquid template params without JSON-escaping filter arguments

### 📚 Documentation

- *(scout)* Add design spec for whatsapp campaign reply tracking
- *(kanban)* Add backlog spec72 for whatsapp campaign reply tracking

### ⚙️ Miscellaneous Tasks

- *(release)* Update changelog for 4.17.1-ichatr.3 [skip ci]
- *(release)* Update changelog for 4.17.1-ichatr.4 [skip ci]

### 💼 Other

- Sync ichatr-main into ichatr-scout
- Merge ichatr-scout into ichatr-main for first scout RC
- Sync upstream v4.18.0 into ichatr-main
...
- *(opportunities)* Search, filter, and total by campaign attribution
- *(opportunities)* Unify card click behavior and add clickable conversation history links
- *(reports)* Add ad campaign performance report

### 🐛 Bug Fixes

- *(dev)* Install missing gems on vite container boot

### 📚 Documentation

- *(kanban)* Add backlog specs for funnel search filters and unified card click/history links
- *(kanban)* Move ciclo 11 backlog specs into dedicated folder
- *(specs)* Add spec-kit planning artifacts for funnel search/filters/totals
- *(kanban)* Move campaign performance spec into ciclo 11 folder
- *(specs)* Add spec-kit planning artifacts for card click/history links and campaign performance report
- *(agents)* Bake develop-freshness and fork-gap-fixing into the release process

### ⚙️ Miscellaneous Tasks

- *(release)* Push branch before tag so run_foss_spec triggers first
- *(release)* Update changelog for 4.16.2-ichatr.3 [skip ci]
- *(dev)* Simulate campaign attribution in opportunity seed script
- *(speckit)* Update toolkit scripts and templates
- *(sync-hooks)* Register missing wiring points for existing fork patches

### 💼 Other

- Sync upstream v4.17.1 into ichatr-main
## [4.17.0-ichatr.1] - 2026-08-20

### 🚀 Features

- *(pipeline_stages)* Allow custom attributes to be required across multiple stages
- *(opportunities)* Implement multi-conversation opportunity lifecycle
- *(opportunities)* Implement opportunity activity audit log and conversation transfer tracking
- *(opportunities)* Implement undo toast notification for kanban moves and status drops

### 🐛 Bug Fixes

- *(specs)* Correct upstream v4.17.0 test failures

### 📚 Documentation

- *(kanban)* Move multi-stage-required-fields to ciclo 9

### ⚙️ Miscellaneous Tasks

- Stage remaining upstream v4.17.0 merge changes

### 💼 Other

- Sync upstream v4.17.0 into ichatr-main
## [4.16.2-ichatr.3] - 2026-08-17
...
- *(broadcasts)* Broadcast opportunity_updated over action cable on opportunity_created
- *(pipeline_stages)* Wrap create/update payloads so required custom attributes persist
