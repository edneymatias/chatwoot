...
- [X] T035 [HIGH] Reported directly by the user: after a Scout conversation is handed off to a human (manual reply or the automatic rescue handoff), `Opportunity#scout_engaged?`'s underlying data flips correctly, but the already-open Kanban board never learns about it in real time — the card keeps showing the "Scout" badge until a manual page reload. Root cause: the badge is derived from the *conversation's* status, but nothing about a conversation-only status change touches the Opportunity record, so `Opportunity#broadcast_opportunity_updated`'s `after_commit on: %i[create update]` (`custom/app/models/opportunity.rb`) never fires; the frontend's `onConversationUpdated` handler (`app/javascript/dashboard/helper/actionCable.js`) also only feeds the plain `conversation` Vuex store, never `opportunities/syncOpportunity`. Fixed by adding `after_commit :refresh_linked_opportunities_scout_badge, on: :update, if: :saved_change_to_status?` to `custom/app/models/custom/concerns/conversation.rb` (the existing, previously-unused `Custom::Concerns::Conversation` extension point, already `include_mod_with`'d into core `Conversation`), which pushes a fresh `opportunity_updated` ActionCable message for every opportunity linked to the conversation via `opportunity_conversations`. Deliberately added `Opportunity#broadcast_scout_badge_refresh` as a *new*, separate public method calling `ActionCableBroadcastJob.perform_later` directly, rather than reusing `.touch`/`broadcast_opportunity_updated`'s `dispatch_event`/Wisper path — verified empirically (via `rails runner`) that going through the existing Wisper dispatcher would also invoke `Custom::AutomationRuleListener#opportunity_updated`, spuriously re-triggering "opportunity updated" automation rules on every conversation status change even when nothing about the opportunity itself changed; the new method bypasses that bus entirely while still reaching the frontend via the same ActionCable channel/event name it already listens for. Also discovered and left alone as pre-existing, unrelated behavior: `Custom::ActionCableListener#conversation_opened` (`custom/app/listeners/custom/action_cable_listener.rb`) already promotes `active_conversation` (and separately broadcasts) when a conversation opens and matches an opportunity with a nil `active_conversation_id` — this fix's broadcast is additive and still necessary for cases that path doesn't cover (manual handoff, non-origin/non-active conversations in multi-conversation deals, and any status transition other than opening into a nil-active-conversation match). Added `custom/spec/models/custom/concerns/conversation_spec.rb` (new file) covering: rebroadcast on status change, payload reflecting the current `scout_engaged?` value, no rebroadcast on unrelated attribute changes, no linked opportunities not erroring, and — using `resolved!` specifically to isolate from the pre-existing `conversation_opened` promotion path — that the fix does not dispatch through `Rails.configuration.dispatcher` (confirming no automation-rule side effect). Verified: RuboCop 0 offenses, 248/248 examples passing across the full affected suite (`conversation_spec.rb`, `custom/message_spec.rb`, `follow_up_job_spec.rb`, `handoff_service_spec.rb`, `agent_runner_spec.rb`, `scout_spec.rb`, `scouts_controller_spec.rb`, `opportunity_spec.rb`, core `conversation_spec.rb`, `schedule_spec.rb`). per FR-013, US4/AC2 (missing)
