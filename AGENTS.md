...
- **Mandatory Pre-Release Verification Checklist** (must run and pass locally before cutting/tagging any release):
  ...
  5. **Full / Comprehensive Backend Test Suite**: Run `docker compose exec rails env -u FRONTEND_URL RAILS_ENV=test bundle exec rspec` (or at minimum the full suite of custom and modified core models/controllers: `custom/spec/ spec/models/opportunity_spec.rb spec/services/automation_rules/conditions_filter_service_spec.rb spec/requests/api/v1/accounts/pipeline_stages_controller_spec.rb`).
     - **Known pending/flaky specs — not new regressions**: `spec/enterprise/services/enterprise/whatsapp/oneoff_campaign_service_spec.rb` (3 examples) and `spec/enterprise/services/enterprise/whatsapp/incoming_message_base_service_spec.rb` (1 example) are permanently `pending`: `Custom::Whatsapp::OneoffCampaignService`/`Custom::Whatsapp::IncomingMessageBaseService` fully intercept (no `super`) the same-named Enterprise modules — `ChatwootApp.extensions` is `%w[enterprise custom]`, so `custom` always wins prepend precedence (see `specs/045-whatsapp-campaign-reply-tracking/research.md`, "Decision: Replace, not augment, the Enterprise recipient tracking") — so Enterprise's own `campaign_recipients` table is permanently empty in this fork. Don't delete these specs or "fix" them by wiring `super`; the comment above each pending `it` explains why. `spec/builders/agent_builder_spec.rb` is separately known to be order-dependent in a full suite run (passes standalone) from `Devise::Mailer` argument-count drift — re-run it alone before treating a failure there as a real regression.
