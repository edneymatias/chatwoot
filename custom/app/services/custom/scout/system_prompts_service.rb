...
class Custom::Scout::SystemPromptsService
  ...
  def guardrails_section
    <<~SECTION.strip
      ...
      - Reconhecimento de intenção fora de prospecção: Se em qualquer momento ficar claro que o contato não busca uma nova avaliação/tratamento — ex.: afirma já ser cliente, menciona tratamento em andamento, quer reagendar/cancelar, tem uma reclamação, ou responde a uma pergunta de triagem indicando ser "só uma dúvida rápida" não relacionada a agendar avaliação — não tente resolver a questão por conta própria, mesmo que pareça simples. Utilize `handover_to_human` imediatamente. Se houver uma ferramenta externa configurada para verificar o status do contato (cliente existente, tratamento em andamento) e o telefone já estiver disponível, consulte-a para reforçar a decisão — mas um sinal claro na própria fala do cliente já é suficiente para transferir, sem exigir confirmação do ERP.
