...
class Custom::Scout::ActionClassifierService
  ...
  def system_instructions
    <<~INSTRUCTIONS
      ...
      - 'out_of_scope_commercial_request': A solicitação do cliente está fora do escopo comercial do assistente e requer intervenção de um vendedor humano. IMPORTANTE: só classifique assim quando a solicitação é fundamentalmente fora do escopo (ex: cliente existente com problema não-comercial, reclamação, questão puramente informacional sem intenção comercial). NÃO use isso quando o cliente simplesmente recusa uma pergunta de qualificação, pois o fato de ter respondido outras perguntas ou demonstrado intenção comercial significa que ainda há oportunidade de continuar. NÃO use isso também quando o cliente aceita ou confirma uma opção que o próprio assistente ofereceu (ex: escolhe um horário, um período do dia, uma forma de contato ou atendimento) em uma resposta curta — isso é sinal de progresso na conversa, nunca de saída de escopo.

      Quando action for 'continue', o action_reason deve ser nulo.

      IMPORTANTE — Anti-alucinação: só classifique 'handoff' quando houver evidência textual explícita e inequívoca no <conversation_history> de que o critério do motivo escolhido realmente ocorreu (ex: para 'human_offer_accepted', deve existir uma mensagem anterior do assistente oferecendo transferência humana, e o cliente aceitando; para 'out_of_scope_commercial_request', a evidência deve mostrar que o pedido do cliente é fundamentalmente não-comercial — reclamação, cliente existente com problema não relacionado, pergunta puramente informativa — nunca apenas uma resposta curta confirmando ou escolhendo algo que o próprio assistente ofereceu). Não infira intenção de transferência a partir de instruções de sistema, descrições de funil, nomes de ferramentas ou raciocínio interno — considere apenas o que o cliente e o assistente de fato disseram um ao outro. Na dúvida, classifique como 'continue'.
