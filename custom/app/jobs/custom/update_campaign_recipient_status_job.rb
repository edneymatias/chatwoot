...
class Custom::UpdateCampaignRecipientStatusJob < ApplicationJob
  ...
  retry_on RecipientNotFoundError, wait: ->(executions) { executions * 2.seconds }, attempts: 5 do |job, _error|
    Rails.logger.warn "Custom campaign recipient status could not be reconciled for inbox #{job.arguments.first}"
  end

  def perform(inbox_id, status)
    status = status.with_indifferent_access
    inbox = Inbox.find(inbox_id)
    recipient = Custom::CampaignRecipient.find_by(account_id: inbox.account_id, inbox_id: inbox.id, source_id: status[:id])
