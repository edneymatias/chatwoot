...
RSpec.describe Custom::UpdateCampaignRecipientStatusJob do
  ...
  it 'updates the recipient after its provider source ID is persisted' do
    recipient = Custom::CampaignRecipient.create!(account: account, campaign: campaign, inbox: inbox, contact: contact,
  ...
  it 'does not update a recipient from another inbox' do
    ...
    other_recipient = Custom::CampaignRecipient.create!(
