    ...
    context 'with opportunity conditions' do
      it 'matches when stage and value conditions match' do
        rule = create(:automation_rule, account: account, event_name: 'opportunity_stage_changed',
                                        conditions: [
                                          { 'attribute_key' => 'pipeline_stage_id', 'filter_operator' => 'equal_to',
                                            'values' => [stage1.id.to_s], 'query_operator' => 'and' },
                                          { 'attribute_key' => 'value', 'filter_operator' => 'greater_than',
                                            'values' => ['1000'], 'query_operator' => 'and' }
                                        ])
      ...
      it 'matches from_pipeline_stage_id when passed in options' do
        rule = create(:automation_rule, account: account, event_name: 'opportunity_stage_changed',
                                        conditions: [
                                          { 'attribute_key' => 'from_pipeline_stage_id', 'filter_operator' => 'equal_to',
                                            'values' => [stage1.id.to_s], 'query_operator' => 'and' }
                                        ])
      ...
      it 'returns false when condition does not match' do
        rule = create(:automation_rule, account: account, event_name: 'opportunity_stage_changed',
                                        conditions: [
                                          { 'attribute_key' => 'value', 'filter_operator' => 'greater_than',
                                            'values' => ['10000'], 'query_operator' => 'and' }
                                        ])
    ...
    context 'with contact and conversation conditions' do
      it 'matches contact email and conversation inbox' do
        rule = create(:automation_rule, account: account, event_name: 'opportunity_created',
                                        conditions: [
                                          { 'attribute_key' => 'email', 'filter_operator' => 'equal_to',
                                            'values' => ['alice@example.com'], 'query_operator' => 'and' },
                                          { 'attribute_key' => 'inbox_id', 'filter_operator' => 'equal_to',
                                            'values' => [conversation.inbox_id.to_s], 'query_operator' => 'and' }
                                        ])
